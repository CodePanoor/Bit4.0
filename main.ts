...
enum YFADOutputModule {
    ...
    LED = 0x0,
    //% blockId="YFDOM_FAN" block="风扇"
    FAN = 0x2,
    //% blockId="YFDOM_VIBRATION_MOTOR" block="振动电机"
    VIBRATION_MOTOR = 0x4,
}
...
enum YFDigitalOutputModule {
    //% blockId="YFDOM_LED" block="LED"
    LED = 0x0,
    //% blockId="YFDOM_BUZZER" block="蜂鸣器"
    BUZZER = 0x1,
    //% blockId="YFDOM_FAN" block="风扇"
    FAN = 0x2,
    //% blockId="YFDOM_RELAY" block="继电器"
    RELAY = 0x3,
    //% blockId="YFDOM_VIBRATION_MOTOR" block="振动电机"
    VIBRATION_MOTOR = 0x4,
}
...
enum YFAudioPlaybackFunWithNum {
    //% blockId="YFAPF_SelectPlay" block="选曲播放"
    SelectPlay = 0x0B,
    //% blockId="YFAPF_SetVolume" block="设置音量"
    SetVolume = 0x0C,
    //% blockId="YFAPF_SetEQ" block="设置EQ模式"
    SetEQ = 0x0D,
    //% blockId="YFAPF_SetPlayMode" block="设置播放模式"
    SetPlayMode = 0x0E,
    //% blockId="YFAPF_SetInsertTrack" block="设置插播曲目"
    SetInsertTrack = 0x10,
}

enum YFAudioPlaybackFun {
    //% blockId="YFAPF_Play" block="播放"
    Play = 0x11,
    //% blockId="YFAPF_Pause" block="暂停"
    Pause = 0x12,
    //% blockId="YFAPF_Stop" block="停止"
    Stop = 0x13,
    //% blockId="YFAPF_PreviousTrack" block="上一曲"
    PreviousTrack = 0x14,
    //% blockId="YFAPF_NextTrack" block="下一曲"
    NextTrack = 0x15,
    //% blockId="YFAPF_PreviousContents" block="上一目录"
    PreviousContents = 0x16,
    //% blockId="YFAPF_NextContents" block="下一目录"
    NextContents = 0x17,
    //% blockId="YFAPF_RemoveNumber" block="清除数字"
    RemoveNumber = 0x0A,
    //% blockId="YFAPF_StopInsertTrack" block="停止插播"
    StopInsertTrack = 0x1C,
}

enum YFMVModule {
    //% blockId="YFMVM_MP3_AUDIO" block="MP3音频"
    MP3_AUDIO,
    //% blockId="YFMVM_VOICE_BROADCAST" block="语音播报"
    VOICE_BROADCAST
}

/*************************  Output - Voice Recorder Module *************************/
enum YFVRMFunction {
    //% blockId="YFVRM_PLAY_ONCE" block="播放一次"
    PLAY_ONCE,
    //% blockId="YFVRM_PLAY_LOOP" block="循环播放（断电停止）"
    PLAY_LOOP,
    //% blockId="YFVRM_SPEAKER_ENABLE" block="喇叭使能"
    SPEAKER_ENABLE,
    //% blockId="YFVRM_SPEAKER_DISABLE" block="喇叭禁用"
    SPEAKER_DISABLE
}

/*************************  Output - Traffic Light LED Mode *************************/
enum YFTrafficLightLED {
    //% blockId="YFTLL_AllTurnOFF" block="全灭"
    AllTurnOFF = 0x00,
    //% blockId="YFTLL_RedLED" block="点亮红灯"
    RedLED = 0x01,
    //% blockId="YFTLL_YellowLED" block="点亮黄灯"
    YellowLED = 0x02,
    //% blockId="YFTLL_GreenLED" block="点亮绿灯"
    GreenLED = 0x03,
}

enum YFAnalogInputModule {
    //% blockId="YFAIM_LIGHT" block="光线传感器"
    LIGHT = 0x0,
    //% blockId="YFAIM_SOUND" block="声音传感器"
    SOUND = 0x1,
    //% blockId="YFAIM_POTENTIOMETER" block="电位器"
    POTENTIOMETER = 0x2,
    //% blockId="YFAIM_SOIL_HUMIDITY" block="土壤湿度传感器"
    SOIL_HUMIDITY = 0x3,
    //% blockId="YFAIM_ROCKER_X" block="X轴-摇杆"
    ROCKER_X = 0x4,
    //% blockId="YFAIM_ROCKER_Y" block="Y轴-摇杆"
    ROCKER_Y = 0x5,
}

enum YFDigitalInputModule {
    //% blockId="YFDIM_MAGNETIC_SENSOR" block="磁控传感器"
    MAGNETIC_SENSOR = 0x0,
    //% blockId="YFDIM_TILT_SENSOR" block="倾角传感器"
    TILT_SENSOR = 0x1,
    //% blockId="YFDIM_INFRARED_PROXIMITY_SENSOR" block="红外接近传感器"
    INFRARED_PROXIMITY_SENSOR = 0x2,
    //% blockId="YFDIM_VIBRATION_SENSOR" block="振动传感器"
    VIBRATION_SENSOR  = 0x3,
    //% blockId="YFDIM_PYROELECTRIC_SENSOR" block="热释电传感器"
    PYROELECTRIC_SENSOR = 0x4,
    //% blockId="YFDIM_TOUCH_SENSOR" block="触摸传感器"
    TOUCH_SENSOR = 0x5,
    //% blockId="YFDIM_BUTTON" block="按键"
    BUTTON = 0x6,
    //% blockId="YFDIM_COLLISION_SWITCH" block="碰撞开关"
    COLLISION_SWITCH = 0x7,
    //% blockId="YFDIM_PATROL_LEFT" block="巡线-左"
    PATROL_LEFT = 0x08,
    //% blockId="YFDIM_PATROL_RIGHT" block="巡线-右"
    PATROL_RIGHT = 0x09,
}
...
enum YFSwitchState {
    //% blockId="YF_OFF" block="关闭"
    OFF = 0x0,
    //% blockId="YF_ON" block="打开"
    ON = 0x1
}
...
//% color="#45b787" weight=10 icon="\uf12e" block="硅谷里智能套件"
namespace YFSENSORS {
    ...
    */
    ...
    //% block="%adom| 连接 %adomPin 切换到 %state || 比例 %percentage \\%"
    ...
    //% block="%dom| 连接 %domPin| %sws=YFSENSORS_domOnOff"
    ...
    //% block="语音播报 %vbmPin| 播放 %serial_number=YFSENSORS_OTPFixedVoiceListNum || 并延时 %delayt| ms"
    ...
    //% block="语音播报 %vbmPin| 连续播放 %serial_number"
    //% vbmPin.fieldEditor="gridpicker" vbmPin.fieldOptions.columns=4
    //% inlineInputMode=inline
    ...
    //% block="音频播放模块选择曲目：%specified_track"
    ...
    //% block="音频播放模块 %specified_fun| %specified_track"
    //% specified_fun.fieldEditor="gridpicker" specified_fun.fieldOptions.columns=2
    ...
    //% block="交通灯 %tlm1Pin| %tlm2Pin| %wColor"
    ...
    //% block="DHT11温湿度传感器 %pin %dht11state 值"
    ...
    //% block="超声波trig|%trig|echo|%echo|单位|%unit"
    ...
    //% block="连接电机驱动 %w_M| DIR %pin_dir| PWM %pin_pwm"
    ...
    //% block="电机|%index|方向|%direction|速度|%speed"
    ...
    //% block="连接红外接收到 %pin| 编码格式：%protocol"
